...
export default function EditEvent() {
    ...
    const { data: eventData } = api.event.getEvent.useQuery({ id: eventID as string }, {
    ...
    useEffect(() => {
        if (eventData) {
            form.reset({
                ...
                maxTeamMembers: eventData.maxTeamMembers,
                dateandtime: {
                    date: new Date(eventData.date),
                    time: moment(eventData.date).toArray().slice(3, 5).map(x => x.toString().padStart(2, '0'))
                }
            });
        }
    }, [ eventData ]);
    ...
    function formSubmit(values: z.infer<typeof formSchema>) {
        const event: Event = {
            ...
            maxTeamMembers:  participation === "SOLO" ? 1 : values.maxTeamMembers,
        };

        eventEdit.mutate({ event: event, id: eventID as string }, {
            onSuccess(data) {
                toast({
        ...
        <>
            ...
            <div className={`flex flex-col items-center w-screen h-screen bg-background text-primary gap-y-5 ${inter}`}>
                ...
                <span className="w-1/3 items-start mt-32">
                    <Link className="text-sm flex items-center hover:underline" href={`/events/${eventID}`}><ChevronLeft className="w-5 h-5" /> Back</Link>
                </span>

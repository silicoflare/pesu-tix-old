...
import { ChevronLeft, Eye, EyeOff, Users, Pencil, Trash, User, Info, Link2, KeySquare, Clipboard, Trash2 } from "lucide-react";
...
export default function ViewEvent() {
    ...
    useQuery({
        ...
        queryFn: () => {
            if (router.isReady && session?.user.studentInfo) {
                server_api.event.checkRegistration.query({ prn: session!.user.studentInfo!.prn, id: ID })
                    .then(res => {
                        // console.log(res);
                        if (res === null) {
                            setIsRegd(false);
    ...
    function toggleRegistration(teamName?: string, teamID?: string) {
        setIsRegd((old: boolean) => {
            let newVal = old;    
            if (old) {
                deregister.mutate({ id: ID, prn: session!.user.id, regID: regInfo?.id || "" }, {
                    onSuccess: () => {
                        setRegInfo(null);
                        setIsRegd(false);
                        toast({
                ...
                register.mutate({ id: ID, prn: session!.user.id, teamName: teamName || undefined, teamID: teamID || undefined }, {
                    onSuccess: (data) => {
                        setRegInfo(data);
                        setIsRegd(true);
                        toast({
    ...
    function kickOut(prn: string, name: string)   {
        deregister.mutate({ id: ID, prn: prn, regID: regInfo?.id || "" }, {
            onSuccess: () => {
                toast({
        ...
        <div className={`window ${montserrat}`}>
            ...
            {formData && (
                <>
                        ...
                        <Authed roles={["club", "admin"]}>
                            <span className="flex flex-row items-center gap-x-2">
                                ...
                                <Popover>
                                    ...
                                    <PopoverContent className="w-[25rem]">
                                        <h1 className="text-xl font-bold w-full text-center">Event Credentials</h1>
                                        <div className="grid grid-cols-[25%,60%,15%] items-center w-full gap-y-3 gap-x-2 mt-3">
                                            <span>Username:</span>
                                            <span className="p-2 border rounded-md text-sm">{formData.id}</span>
                                            <span>
                                                <Button variant="outline" className="w-10 p-1" onClick={() => yoink(formData.id, "username")}><Clipboard className="w-5 h-5" /></Button>
                                            </span>
                                            <span>Password:</span>
                                            <span className="p-2 border rounded-md text-sm">{formData.password}</span>
                                            <span className="flex gap-x-2">
                                                <Button variant="outline" className="w-10 p-1" onClick={() => yoink(formData.password, "password")}><Clipboard className="w-5 h-5" /></Button>
                                            </span>
                                        </div>
                                    </PopoverContent>
                                </Popover>
                    ...
                    <span className="w-1/3">{moment(formData.date).format("LL \\at LT")}</span>
                    <div className="w-1/3 my-3 mt-5 flex items-center justify-between">
                        {
                            formData.participation === "SOLO" ? (
                                <Button variant="outline"><User />Solo</Button>
                            ) : (
                                <Button variant="outline" className="cursor-default hover:bg-background flex items-center gap-x-3"><Users />Teams of {formData.maxTeamMembers}</Button>
                            )
                        }
                        <Badge>{formData.type}</Badge>
                    </div>
                    <Image src={ imgUrl === "" ? "/dummy_event.jpeg" : imgUrl } className="w-1/3 py-5" height={500} width={500} alt="event image" />
                    <div className="w-1/3 mt-10 text-lg" dangerouslySetInnerHTML={{ __html: formData.description }}></div>
                    <Authed roles={["student"]}>
                        {
                            (regInfo === null || (regInfo?.regType === "SOLO" || (regInfo?.regType === "TEAM" && !isRegd))) ? (
                                <Popover>
                                    <PopoverTrigger asChild>
                                    ...
                                    <PopoverContent className="w-[25rem]">
                                        {
                                            formData.participation === "SOLO" ? (
                                                ...
                                                <div className="flex flex-col items-center w-full">
                                                    <Tabs defaultValue={regTeamType} className="w-full bg-transparent">
                                                        <TabsList className="w-full grid grid-cols-2 bg-transparent gap-2">
                                                            <TabsTrigger value="create" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground border rounded-md">Create Team</TabsTrigger>
                                                            <TabsTrigger value="join" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground border rounded-md">Join Team</TabsTrigger>
                                                        </TabsList>
                                                        <TabsContent value="create">
                                                            <div className="flex flex-col items-center w-full">
                                                                <span className="w-full text-center text-xl font-semibold">Create Team</span>
                                                                <span className="w-full flex items-center">
                                                                    Team Name:
                                                                    <Input value={regTeamName} onChange={(e) => setRegTeamName(e.target.value)} />
                                                                </span>
                                                                <br />
                                                                ...
                                                                <PopoverClose asChild>
                                                                    <Button onClick={() => toggleRegistration(regTeamName)}>Yes</Button>
                                                                </PopoverClose>
                                                            </div>
                                                        </TabsContent>
                                                        <TabsContent value="join">
                                                            <div className="flex flex-col items-center w-full">
                                                                <span className="w-full text-center text-xl font-semibold">Join Team</span>
                                                                <span className="w-full flex items-center">
                                                                    Team ID:
                                                                ...
                                                                <PopoverClose asChild>
                                                                    <Button onClick={() => toggleRegistration(undefined, regTeamID)}>Yes</Button>
                                                                </PopoverClose>
                                                            </div>
                                                        </TabsContent>
                                                    </Tabs>
                                ...
                                <div className="flex flex-col items-center w-1/3 p-5 rounded-md border mt-10">
                                    <span className="w-full text-center text-xl font-semibold">{regInfo?.teamName}</span>
                                    <span className="w-full grid grid-cols-2 gap-3 px-5 my-3">
                                        <Button onClick={() => yoink(`${regInfo?.id}`, "Team ID")}><Info />&nbsp;Copy Team ID</Button>
                                        <Button onClick={() => yoink(`${process.env.NEXT_PUBLIC_URL}/events/${ID}?teamID=${regInfo?.id}`, "Team Link")}><Link2 />&nbsp;Copy Team Link</Button>
                                    </span>
                                    <span className="w-full grid grid-cols-2 items-center my-3 px-5 gap-2">
                                        {
                                            regInfo?.students.map((student) => (
                                                <span key={student.prn} className="w-full text-center flex items-center justify-center gap-x-3" style={{ color: regInfo.ownerID === student.prn ? "green" : "" }}>
                                                    {namify(student.name)}
                                                    { (regInfo.ownerID === session?.user.id && student.prn !== regInfo.ownerID) && (
                                                        <Popover>
                                                            <PopoverTrigger asChild>
                                                                <Button variant="ghost"><Trash2 className="w-5 h-5 text-white" /></Button>
                                                            </PopoverTrigger>
                                                            <PopoverContent className="w-[25rem]">
                                                                <div className="flex flex-col items-center w-full">
                                                                    <span className="w-full text-center flex flex-col text-xl">
                                                                        Are you sure you want to kick {student.name} from this team?
                                                                    </span>
                                                                    <span className="flex flex-row justify-center items-center w-full gap-x-5 mt-5">
                                                                        <PopoverClose asChild>
                                                                            <Button onClick={() => kickOut(student.prn, student.name)}>Yes</Button>
                                                                        </PopoverClose>
                                                                        <PopoverClose asChild>
                                                                            <Button variant="outline">No</Button>
                                                                        </PopoverClose>
                                    ...
                                    <Popover>
                                        <PopoverTrigger asChild>
                                            <Button variant="destructive" className="w-1/3">Unregister</Button>
                                        </PopoverTrigger>
                                        <PopoverContent className="w-[25rem]">
                                            <div className="flex flex-col items-center w-full">
                                                <span className="w-full text-center flex flex-col text-xl">
                                                ...
                                                <span className="flex flex-row justify-center items-center w-full gap-x-5 mt-5">
                                                    <PopoverClose asChild>
                                                        <Button onClick={() => toggleRegistration(undefined, regInfo?.id)}>Yes</Button>
                                                    </PopoverClose>
                                                    <PopoverClose asChild>
                                                        <Button variant="outline">No</Button>
                                                    </PopoverClose>
